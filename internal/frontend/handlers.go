...
func (s *Server) handleUpgradeCheck(w http.ResponseWriter, r *http.Request) {
	...
	result := upgradeCheckResponse{
		...
		UpdateAvailable: release.TagName != currentVersion && release.TagName != "",
	}
...
func (s *Server) handleUpgradeDownload(w http.ResponseWriter, r *http.Request) {
	...
	var req upgradeDownloadRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		s.writeErrorStatus(w, http.StatusBadRequest, "invalid JSON body")
	...
	if req.URL == "" {
		s.writeErrorStatus(w, http.StatusBadRequest, "url is required")

		return
	}

	upgradeState.mu.Lock()
	if upgradeState.state == "downloading" {
	...
	upgradeState.state = "downloading"
	upgradeState.progress = 0
	upgradeState.err = ""
	upgradeState.mu.Unlock()

	go s.downloadFirmware(req.URL, req.SHA256)

	s.writeJSON(w, map[string]string{"status": "downloading"})
}

func (s *Server) downloadFirmware(url, expectedSHA string) {
	setError := func(msg string) {
		upgradeState.mu.Lock()
		upgradeState.state = "error"
		upgradeState.err = msg
		upgradeState.mu.Unlock()
	}
	...
	for {
		...
		if n > 0 {
			...
			if totalSize > 0 {
				upgradeState.mu.Lock()
				upgradeState.progress = int(downloaded * 100 / totalSize)
				upgradeState.mu.Unlock()
			}
		}
	...
	if expectedSHA != "" {
		...
		if !strings.EqualFold(actualSHA, expectedSHA) {
			setError(fmt.Sprintf("SHA256 mismatch: expected %s, got %s", expectedSHA, actualSHA))

			_ = os.Remove("/tmp/sysupgrade.bin")
...
func (s *Server) handleUpgradeApply(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := os.Stat("/tmp/sysupgrade.bin"); os.IsNotExist(err) {
		s.writeErrorStatus(w, http.StatusBadRequest, "no firmware file found at /tmp/sysupgrade.bin")
...
func (s *Server) handleUpgradeUpload(w http.ResponseWriter, r *http.Request) {
	...
	// Limit upload to 256 MB.
	const maxUploadSize = 256 << 20

	r.Body = http.MaxBytesReader(w, r.Body, maxUploadSize)

	if err := r.ParseMultipartForm(maxUploadSize); err != nil {
		s.writeErrorStatus(w, http.StatusBadRequest, "file too large or invalid multipart form")
	...
	if err != nil {
		s.writeErrorStatus(w, http.StatusBadRequest, "missing 'file' field in upload")
	...
	if _, err := io.Copy(outFile, file); err != nil {
		s.writeErrorStatus(w, http.StatusInternalServerError, "failed to save firmware file")
	...
	s.writeJSON(w, map[string]string{"status": "uploaded"})
}
